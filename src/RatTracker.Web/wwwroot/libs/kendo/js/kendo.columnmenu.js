	                                        ...
	                                        '<div class="k-columnmenu-item-content k-columns-item">' +
	                                            '<div class="k-column-list-wrapper">'+
	                                                ...
	                                                '#for (var idx = 0; idx < columns.length; idx++) {#'+
	                                                    ...
	                                                    '#} else {#' +
	                                                        '<label class="k-column-list-item" role="menuitemcheckbox" aria-checked="false" #=columns[idx].matchesMedia === false ? "style=\'display:none;\'" : ""#><input class="k-checkbox" type="checkbox" title="#=encodeTitles ? kendo.htmlEncode(columns[idx].title) : columns[idx].title#" data-#=ns#field="#=columns[idx].field.replace(/\"/g,"&\\#34;")#" data-#=ns#index="#=columns[idx].index#" data-#=ns#locked="#=columns[idx].locked#" data-#=ns#uid="#=columns[idx].uid#"/><span class="k-checkbox-label"> #=encodeTitles ? kendo.htmlEncode(columns[idx].title) : columns[idx].title#</span></label>'+
	                                                    '#}#'+
	                                                '#}#'+
	                                                '</div>' +
	                                            '</div>'+
	                                            '<div class="k-columnmenu-actions">' +
	                                                '<button class="k-button" type="button">${messages.reset}</button>' +
	                                                '<button class="k-button k-primary" type="button">${messages.apply}</button>' +
	                                            '</div>' +
	                                        '</div>' +
	                                    '</div>' +
	                                '</div>' +
