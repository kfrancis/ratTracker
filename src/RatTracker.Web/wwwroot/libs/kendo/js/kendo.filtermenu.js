...
/***/ (function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;(function(f, define){
	    !(__WEBPACK_AMD_DEFINE_ARRAY__ = [ __webpack_require__(1189), __webpack_require__(1106), __webpack_require__(1064), __webpack_require__(1105) ], __WEBPACK_AMD_DEFINE_FACTORY__ = (f), __WEBPACK_AMD_DEFINE_RESULT__ = (typeof __WEBPACK_AMD_DEFINE_FACTORY__ === 'function' ? (__WEBPACK_AMD_DEFINE_FACTORY__.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__)) : __WEBPACK_AMD_DEFINE_FACTORY__), __WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));
	})(function(){
	        ...
	        '<div class="k-filter-menu-container">' +
	            '<div>' +
	                '<ul class="k-radio-list k-reset">' +
	                    '<li>' +
	                        '#inputId = kendo.guid()#' +
	                        '<input type="radio" class="k-radio" id="#=inputId#" data-#=ns#bind="checked: filters[0].value" value="true" name="filters[0].value" />' +
	                        '<label class="k-radio-label" for="#=inputId#">#=messages.isTrue#</label>' +
	                    '</li>' +
	                    '<li>' +
	                        '#inputId = kendo.guid()#' +
	                        '<input type="radio" class="k-radio" id="#=inputId#" data-#=ns#bind="checked: filters[0].value" value="false" name="filters[0].value" />' +
	                        '<label class="k-radio-label" for="#=inputId#">#=messages.isFalse#</label>' +
	                    '</li>' +
	                '</ul>' +
	                '<div class="k-columnmenu-actions">' +
	                    '<button class="k-button" type="reset" title="#=messages.clear#">#=messages.clear#</button>' +
	                    '<button class="k-button k-primary" type="submit" title="#=messages.filter#">#=messages.filter#</button>' +
	                '</div>' +
	            '</div>' +
	        '</div>';
	        ...
	        _createLink: function() {
	            ...
	            if (!link[0]) {
	                link = element.prepend('<a class="k-grid-filter" href="#" title="' +
	                    title + '" aria-label="' +
	                    title + '"><span class="k-icon k-i-filter"></span></a>').find(".k-grid-filter");
	            }
	    ...
	    var FilterMultiCheck = Widget.extend({
	        ...
	        _createForm: function() {
	            ...
	            if (!this._isMobile) {
	                ...
	                if (options.search) {
	                    html += "<span class='k-searchbox'>" +
	                                "<span class='k-input-prefix'>" +
	                                    "<span class='k-icon k-i-search'></span>" +
	                                "</span>" +
	                                "<input class='k-input' type='text' placeholder='"+ options.messages.search + "' />" +
	                                "<span class='k-input-suffix'>" +
	                                    "<button type='button' class='k-button k-icon-button k-button-clear'><span class='k-icon k-i-close'></span></button>"+
	                                "</span>" +
	                            "</span>";
	                }
