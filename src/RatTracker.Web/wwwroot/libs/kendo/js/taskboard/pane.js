	    ...
	    var TaskBoardPane = Observable.extend({
	        ...
	        headerTemplate: '<div class="#:styles.headerText#">{0}</div>' +
	                        ...
	                        '<div class="#:styles.headerActions#">' +
	                            '<button class="#:styles.button#" title="#:messages.close#" #:kendo.attr("command")#="ClosePaneCommand">' +
	                                '<span class="k-icon k-i-close"></span>' +
	                            '</button>' +
	                        '</div>',
	        buttonTemplate: '<button class="#:styles.footerButton##if(primary){# #:styles.primary##}#" title="#:text#" #:kendo.attr("command")#="#:command#" #:kendo.attr("options")#="#:options#">' +
	                            '<span class="#:icon# #:spriteCssClass#"></span>#:text#' +
	                        '</button>',
	        contentTemplate: '',
	        builtinButtons: {
